...
def display_metadata(last_downloaded_titles):
    mm = MetadataManager()
...
def main():
    ...
    checker = DownloaderManager(videos, downloader)
    ...
    if downloaded_titles:  # Only if there are new downloaded videos
        message = f"All downloads completed. {len(downloaded_titles)} videos downloaded."
        print(message)
        logger.info(message)
        
        # Check if user wants to view the metadata
        choice = input("Would you like to view the metadata for downloaded videos? (y/n): ")
        if choice.lower() == 'y':
            display_metadata(downloaded_titles)  # Pass downloaded titles to the function
    else:
        message = f"All videos already downloaded. {len(downloaded_titles)} videos checked."
        print(message)
        logger.info(message)
